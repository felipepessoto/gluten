 ...
 * variant) and delegates such writes to the vanilla Delta write path. These tests use offloaded
 * writes (save and UPDATE) so they exercise that path; with INSERT INTO the write is not offloaded
 * and would pass regardless of the fix.
 */
