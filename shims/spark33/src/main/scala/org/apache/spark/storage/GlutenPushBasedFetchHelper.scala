...
private class GlutenPushBasedFetchHelper(
  ...
  def sendFetchMergedStatusRequest(req: FetchRequest): Unit = {
    ...
    val mergedBlocksMetaListener = new MergedBlocksMetaListener {
      override def onSuccess(
        ...
        try {
          iterator.addToResultsQueue(PushMergedRemoteMetaFetchResult(
            shuffleId,
            shuffleMergeId,
            reduceId,
   ...
   *   3. There is a failure when processing SuccessFetchResult which is for a shuffle chunk (local
   *      or remote).
   *   4. There is a zero-size buffer when processing SuccessFetchResult for a shuffle chunk (local
   *      or remote).
   */
